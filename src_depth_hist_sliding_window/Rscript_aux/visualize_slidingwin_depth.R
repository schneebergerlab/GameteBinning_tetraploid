...
#
path <- "/netscratch/dep_mercier/grp_schneeberger/projects/Potato_single_cells/reference_manish_assembled_haplotye_aware/0_read_alignment_rate_checking/"

# Real assembly size: 2.437906533 Gb
ctgsizes    <- read.table("/netscratch/dep_mercier/grp_schneeberger/projects/Potato_single_cells/reference_manish_assembled_haplotye_aware/potato.hifiasm.p_utg.contigsizes")
observed_assembly_size <- sum(ctgsizes$V2)

pdf(paste(path, "/multiple_slidingwin_depth_comp_hifiasm.pdf", sep=""), height=8, width=14.26)
